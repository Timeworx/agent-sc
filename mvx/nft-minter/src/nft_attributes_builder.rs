...
pub const COLLECTION_HASH_LEN: usize = 59;
...
pub enum NftId {
    UniqueId(UniqueId),
    PrespecifiedId(BoxedBytes)
}

impl multiversx_sc::formatter::SCDisplay for NftId {
    fn fmt<F: multiversx_sc::formatter::FormatByteReceiver>(&self, f: &mut F) {
        match self {
            NftId::UniqueId(unique_id) => unique_id.fmt(f),
            NftId::PrespecifiedId(bytes) => bytes.as_slice().fmt(f)
        }
    }
}
