...
use cosmwasm_std::{
    Binary, Deps, DepsMut, Env, MessageInfo, Response, StdError, StdResult, Uint128, to_json_binary, BankMsg, Coin, Addr,
};
...
use crate::msg::{ExecuteMsg, InstantiateMsg, MigrateMsg, QueryMsg, ClaimableResponse, StakeResponse, UnstakeResponse, ConfigMsg, UnstakeItem};
use crate::state::{
    MinterData, TokenInfo, ALLOWANCES, ALLOWANCES_SPENDER, BALANCES, LOGO, MARKETING_INFO,
    TOKEN_INFO, STAKE_INFO, StakeInfo, UNSTAKE_INFO, UnstakeInfo, Config, CONFIG,
};
...
pub fn instantiate(
    ...
    let config = Config {
        apr: msg.config.apr,
        tix_denom: msg.config.tix_denom,
        unstake_duration: msg.config.unstake_duration,
    };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Stake { address } => execute_stake(deps, env, info, address),
        ExecuteMsg::ClaimAndStake { address } => execute_claim_and_stake(deps, env, info, address),
        ExecuteMsg::RestakeAndStake { address } => execute_restake_and_stake(deps, env, info, address),
        ExecuteMsg::Claim {} => execute_claim(deps, env, info),
        ExecuteMsg::Restake {} => execute_restake(deps, env, info),
        ExecuteMsg::Unstake { value } => execute_unstake(deps, env, info, value),
...
pub fn execute_transfer(
    ...
    let mut mut_deps = deps;
    transfer_impl(&mut mut_deps, &env, &info.sender, &recipient, amount)
}
...
pub fn transfer_impl(
    ...
    let source_addr = address.to_string();
    ...
    let claim_result: Response;

    // We don't need to claim if it's a normal add stake / remove stake op
    if address == env.contract.address || rcpt_addr == env.contract.address {
        claim_result = Response::new();
    } else {
        claim_result = claim_impl(deps, env, &source_addr)?;
    }
    ...
    if address != env.contract.address {
        STAKE_INFO.update(
            deps.storage,
            address,
            |stakes: Option<Vec<StakeInfo>>| -> StdResult<_> {
                let stakes = stakes.unwrap_or_default();
                if stakes.len() != 1 {
                    Err(StdError::NotFound { kind: String::from("Stake") })
                } else {
                    let stake = stakes.first().unwrap();
                    Ok(
                        [StakeInfo {
                            value: stake.value - amount,
                            last_claim_ts: env.block.time.seconds()
                        }].to_vec()
                    )
                }
            }
    ...
    let add_stake_result: Response;
    if rcpt_addr != env.contract.address {
        add_stake_result = add_stake(deps, env, recipient, amount)?;
    } else {
        add_stake_result = Response::new();
    }

    let res = claim_result
        .add_attribute("action", "transfer")
        .add_attribute("from", address)
        .add_attribute("to", recipient)
        .add_attribute("amount", amount)
        .add_attributes(add_stake_result.attributes);
...
pub fn execute_send(
    ...
    let mut mut_deps = deps;
    send_impl(&mut mut_deps, &env, info, contract, amount, msg)
}
...
pub fn send_impl(
    ...
    let source_address = info.sender;
    let source_addr = source_address.to_string();
    let recipient = &contract;
    // We don't need to claim if it's a normal add stake / remove stake op
    if source_address == env.contract.address || rcpt_addr == env.contract.address {
        claim_result = Response::new();
    } else {
        claim_result = claim_impl(deps, &env, &source_addr)?;
    }

    if source_address != env.contract.address {
        STAKE_INFO.update(
            deps.storage,
            &source_address,
            |stakes: Option<Vec<StakeInfo>>| -> StdResult<_> {
    ...
    let res = claim_result
        ...
        .add_attribute("from", &source_address)
        .add_attribute("to", &contract)
        .add_attribute("amount", amount)
        .add_attributes(add_stake_result.attributes)
        .add_message(
            Cw20ReceiveMsg {
                sender: source_address.into(),
                amount,
                msg,
            }
...
pub fn add_stake(
    ...
    let mut stake_infos = stake_infos(&deps.as_ref(), address)?;
    let new_stake_info = StakeInfo { value: value, last_claim_ts: env.block.time.seconds() };
    stake_infos.push(new_stake_info);
    STAKE_INFO.save(deps.storage, &deps.api.addr_validate(&address)?, &stake_infos)?;
...
pub fn execute_stake(
    ...
    let config: Config = CONFIG.load(mut_deps.storage)?;

    if coin.denom != config.tix_denom {
        return Err(ContractError::WrongStake {})
    }

    let address = address.unwrap_or(info.sender.clone().into_string());

    // This adds the stake as well
    let transfer_result = transfer_impl(&mut mut_deps, &env, &env.contract.address, &address, coin.amount)?;

    Ok(
        transfer_result
            .add_attribute("stake", coin.amount)
    )
}
...
pub fn execute_claim_and_stake(
    ...
    let claim_result = claim_impl(&mut mut_deps, &env, &address)?;
    ...
    Ok(
        claim_result
            .add_attribute("stake", coin.amount)
            .add_attributes(transfer_result.attributes)
    )
}

pub fn execute_restake_and_stake(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    address: Option<String>
    ...
    let restake_result = restake_impl(&mut mut_deps, &env, &address)?;
...
pub fn execute_claim(
    ...
    let mut mut_deps = deps;
    claim_impl(&mut mut_deps, &env, &info.sender.into_string())
}
...
pub fn claim_impl(
    ...
    let claimable = query_claimable(deps.as_ref(), env.clone(), &address)?.claimable;

    let claim_msg = BankMsg::Send {
        to_address: address.clone(),
        amount: [Coin::new(claimable.into(), config.tix_denom)].to_vec()
    };

    let stake_infos = stake_infos(&deps.as_ref(), &address)?;
    let mut new_stake_infos: Vec<StakeInfo> = Vec::new();

    let block_time = env.block.time.seconds();
    for si in stake_infos {
        new_stake_infos.push(StakeInfo { value: si.value, last_claim_ts: block_time });
    }

    STAKE_INFO.save(deps.storage, &addr, &new_stake_infos)?;

    let merge_result = merge_stakes(deps, env, &address)?;

    Ok(
        Response::new()
            .add_message(claim_msg)
            .add_attributes(merge_result.attributes)
    )
}
...
pub fn execute_restake(
    ...
    let mut mut_deps = deps;
    restake_impl(&mut mut_deps, &env, &info.sender.into_string())
}
...
pub fn restake_impl(
    ...
    let transfer_result = transfer_impl(deps, env, &env.contract.address, address, claimable)?;

    Ok(
        Response::new()
            .add_attributes(merge_result.attributes)
            .add_attributes(transfer_result.attributes)
            .add_attribute("restake", claimable)
    )
}
...
pub fn execute_unstake(
    ...
    let address = addr.clone().into_string();

    let claim_result = claim_impl(&mut mut_deps, &env, &address)?;

    let stakes = stake_infos(&mut_deps.as_ref(), &address)?;
    if stakes.len() != 1 {
        return Err(ContractError::WrongNumberOfStakes {})
    }
    let stake = stakes.first().unwrap();
    if stake.value < value {
        return Err(ContractError::InsufficientStake {})
    }
    let transfer_result = transfer_impl(&mut mut_deps, &env, &addr, &env.contract.address.to_string(), value)?;

    let mut unstakes = unstake_infos(&mut_deps.as_ref(), &address)?;
    unstakes.push(UnstakeInfo { unstake_ts: env.block.time.seconds(), value: value });
    UNSTAKE_INFO.save(mut_deps.storage, &addr, &unstakes)?;

    Ok(
        claim_result
            .add_attribute("unstake", value)
            .add_attributes(transfer_result.attributes)
    )
}
...
pub fn execute_withdraw(
    ...
    let config: Config = CONFIG.load(deps.storage)?;
    let cur_ts = env.block.time.seconds();

    let unstakes = unstake_infos(&deps.as_ref(), &address)?;
    let unstake_count = unstakes.len();
    let mut new_unstakes: Vec<UnstakeInfo> = Vec::new();

    let mut to_return = Uint128::zero();

    for unstake in unstakes {
        if unstake.unstake_ts + config.unstake_duration <= cur_ts {
            to_return += unstake.value;
        } else {
            new_unstakes.push(unstake)
        }
    }

    if new_unstakes.len() < unstake_count {
        UNSTAKE_INFO.save(deps.storage, &addr, &new_unstakes)?;
    }

    if !to_return.is_zero() {
        let withdraw_msg = BankMsg::Send {
            to_address: address,
            amount: [Coin::new(to_return.into(), config.tix_denom)].to_vec()
        };

        Ok(
            Response::new()
                .add_message(withdraw_msg)
                .add_attribute("withdraw", to_return)
        )
    } else {
...
pub fn merge_stakes(
    ...
    // This doesn't work as merging destroys intermediary rewards (< 1 day, that weren't claimed)
    // let claimable = query_claimable(deps.as_ref(), env.clone(), address)?.claimable;
    ...
    let new_stake_info = StakeInfo { value: balance, last_claim_ts: env.block.time.seconds() };
    STAKE_INFO.save(deps.storage, &deps.api.addr_validate(address)?, &[new_stake_info].to_vec())?;

    Ok(
        Response::new()
            .add_attribute("merged", balance)
    )
}
...
pub fn execute_update_config(
    ...
    let token_info: TokenInfo = TOKEN_INFO.load(deps.storage)?;
    if info.sender != token_info.mint.unwrap().minter {
        return Err(ContractError::Unauthorized {});
    }
    let config = Config { 
        apr: config.apr,
        tix_denom: config.tix_denom,
        unstake_duration: config.unstake_duration,
    };
    CONFIG.save(deps.storage, &config)?;
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetClaimable { address} => to_json_binary(&query_claimable(deps, env, &address)?),
        QueryMsg::GetStake { address } => to_json_binary(&query_stake(deps, env, address)?),
        QueryMsg::GetUnstake { address } => to_json_binary(&query_unstake(deps, env, address)?),
    }
}
...
pub fn query_claimable(deps: Deps, env: Env, address: &String) -> StdResult<ClaimableResponse> {
    let stake_infos = stake_infos(&deps, address)?;

    let mut claimable = Uint128::zero();

    for stake_info in &stake_infos {
        claimable += get_stake_claimable(&deps, &env, stake_info)?;
    }

    Ok(ClaimableResponse { claimable: claimable })
}

pub fn query_stake(deps: Deps, env: Env, address: String) -> StdResult<StakeResponse> {
    let total_claimable = query_claimable(deps, env, &address)?;
    let stake_infos = stake_infos(&deps, &address)?;
    let total_stake = stake_infos.iter().map(|si| si.value).sum();

    Ok(StakeResponse { value: total_stake, claimable: total_claimable.claimable })
}

pub fn query_unstake(deps: Deps, env: Env, address: String) -> StdResult<UnstakeResponse> {
    let config: Config = CONFIG.load(deps.storage)?;
    let cur_ts = env.block.time.seconds();

    Ok(
        UnstakeResponse { 
            unstakes: unstake_infos(&deps, &address)?.iter().map(|unstake| UnstakeItem {
                value: unstake.value,
                unstake_ts: unstake.unstake_ts,
                can_withdraw: unstake.unstake_ts + config.unstake_duration <= cur_ts
            }).collect::<Vec<_>>()
        }
    )
}
...
fn stake_infos(
    ...
    match STAKE_INFO.load(deps.storage, &deps.api.addr_validate(&address)?) {
...
fn unstake_infos(
    deps: &Deps,
    address: &String,
) -> StdResult<Vec<UnstakeInfo>> {
    match UNSTAKE_INFO.load(deps.storage, &deps.api.addr_validate(&address)?) {
        Ok(unstake_info) => Ok(unstake_info),
        Err(_error) => Ok(Vec::new())
    }
}

fn get_stake_claimable(
    deps: &Deps,
    env: &Env,
    stake_info: &StakeInfo,
) -> StdResult<Uint128> {
    let config: Config = CONFIG.load(deps.storage)?;
    let apr = Uint128::from(config.apr);

    let now = env.block.time.seconds();

    Ok(Uint128::from(now - stake_info.last_claim_ts) * stake_info.value * apr / Uint128::from(31536000000_u128))
}
