...
use cosmwasm_std::{
    to_binary, from_binary, Addr, Binary, Deps, DepsMut, Empty, Env, MessageInfo, Reply, ReplyOn, Response,
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::ForAddress { address } => execute_mint(deps, info, address),
        ExecuteMsg::ForSender {} => execute_mint(deps, info.clone(), info.sender.into_string()),
    }
}
...
pub fn execute_mint(
    ...
    if config.tix_denom != coin.denom {
    ...
    if coin.amount != config.unit_price {
        return Err(ContractError::WrongPaymentAmount {});
    }

    let token_id = config.unused_token_id.to_string();
    let mut extension = config.extension.clone().unwrap();
    extension.image = Some(extension.image.unwrap().replace("{{TOKEN_ID}}", token_id.as_str()));
    extension.name = Some(extension.name.unwrap().replace("{{TOKEN_ID}}", token_id.as_str()));
...
mod tests {
    ...
    use cosmwasm_std::{from_binary, to_binary, CosmosMsg, SubMsgResponse, SubMsgResult, Coin};
    ...
    fn initialization() {
        ...
        let msg = InstantiateMsg {
            ...
            token_code_id: 10u64,
            tix_denom: String::from("factory/inj1rw3qvamxgmvyexuz2uhyfa4hukvtvteznxjvke/tix"),
    ...
    fn mint() {
        ...
        let info = mock_info(MOCK_CONTRACT_ADDR, &[Coin::new(1, String::from("factory/inj1rw3qvamxgmvyexuz2uhyfa4hukvtvteznxjvke/tix"))]);
        let res = execute(deps.as_mut(), mock_env(), info, ExecuteMsg::ForSender {}).unwrap();
    ...
    fn unauthorized_token() {
        ...
        let info = mock_info("unauthorized-token", &[Coin::new(1, String::from("factory/inj1rw3qvamxgmvyexuz2uhyfa4hukvtvteznxjvke/tix"))]);
        let msg = ExecuteMsg::ForSender {};
        let err = execute(deps.as_mut(), mock_env(), info, msg).unwrap_err();
